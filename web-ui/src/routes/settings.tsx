...
import { authStore, setLoadingProviders, setProviders, startLoginFlow } from "@/stores/auth";
		...
		<>
			<Card className="mt-4">
				<CardHeader>
					...
					<CardDescription>Configure authentication for AI providers (OpenAI, Anthropic, etc.)</CardDescription>
				</CardHeader>
				<CardContent>
					{isLoadingProviders ? (
						...
						<p className="text-sm text-muted-foreground py-4">No providers available.</p>
					) : (
						<div className="space-y-3">
							{providers.map((provider) => (
								<ProviderCard
									...
									onLogin={() =>
										provider.type === "oauth" ? handleOAuthLogin(provider.id) : handleApiKeyLogin(provider.id)
									}
			...
			<div className="flex items-start justify-between gap-4">
				<div className="flex-1">
					<div className="flex items-center gap-2 mb-1">
						...
						<Badge variant={provider.type === "oauth" ? "default" : "secondary"} className="text-xs">
							{provider.type === "oauth" ? (
								<>
					...
					<p className="text-xs text-muted-foreground">{provider.hasAuth ? "Authenticated" : "Not configured"}</p>
					...
					{isEditing && provider.type === "apikey" && (
						<div className="mt-3 space-y-2">
							...
							<div className="flex gap-2">
								<Button size="sm" onClick={onSaveApiKey} disabled={!apiKeyValue.trim()}>
									Save
								</Button>
