...
function parseSchedule(schedule: string): { type: "once" | "recurring"; schedule: string; nextRun: string } {
  ...
  if (relativeMatch) {
    ...
    const ms = unit.startsWith("m") ? amount * 60_000
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      schedule: Type.String({
        description:
          'When to run. Examples: "in 10m", "every 2h", "daily at 9:00", "0 9 * * 1-5"',
      }),
    }),
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `✓ Scheduled ${typeLabel.toLowerCase()} task: "${job.description}"\n` +
                `  ID: ${job.id}\n` +
                `  Next run: ${nextStr}\n` +
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      showCompleted: Type.Optional(
        Type.Boolean({ description: "Include completed one-time jobs (default: false)" })
      ),
    }),
    async execute(_toolCallId, params) {
      const jobs = loadJobs();
      const filtered = params.showCompleted
        ? jobs
        : jobs.filter((j) => !j.completed);
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      if (idx === -1) {
        return {
          content: [
            { type: "text" as const, text: `No job found with ID "${params.id}".` },
          ],
          details: { found: false },
          isError: true,
        };
