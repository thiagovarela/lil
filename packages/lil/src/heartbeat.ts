...
import { loadJobs, saveJobs, calculateNextRun, type CronJob } from "./extensions/cron/index.ts";
...
export class HeartbeatService {
  ...
  start(): void {
    ...
    console.log(
      `[heartbeat] Started (interval: ${this.intervalMs / 60_000}m)`
    );
  ...
  private async executeDueJobs(): Promise<void> {
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        console.error(
          `[heartbeat] Cron job ${job.id} failed:`,
          err instanceof Error ? err.message : String(err)
        );
      }
